...
ggsave(filename = "fig-tab/ivd/eg2.eps", plot = p1, device = cairo_ps, width = 12, height = 12)
...
ggsave(filename = "fig-tab/ivd/eg2-comp1.eps", plot = p2, device = cairo_ps, width = 18, height = 6)
p2d1 <- p2d+ggtitle("Comparison with other methods")
ggsave(filename = "fig-tab/ivd/eg2-comp2.eps", plot = p2d1, device = cairo_ps, width = 8, height = 8)
p2d2 <- p2d+ggtitle("D. Comparison scenario 4")
p24 <- grid.arrange(p2a, p2b, p2c, p2d2, ncol=2)
ggsave(filename = "fig-tab/ivd/eg2-comp.eps", plot = p24, device = cairo_ps, width = 12, height = 12)
...
p3a <- ggplot(dfk2a, aes(x = p, y = est, color = grp, linetype = grp)) +
  geom_line(size=1) +
  scale_y_continuous(limits = c(0.6,0.9), name = "SAUC") +
  scale_x_reverse(n.breaks = 10, name="Marginal selection probability") + 
  theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        panel.grid.major = element_line(colour = "grey87"),
        legend.key = element_rect (fill = "white"),
...
ggsave(filename = "fig-tab/ivd/eg2-k.eps", plot = p3, device = cairo_ps, width = 18, height = 12)
