...
setEPS(width = 10, height = 4); postscript("fig-tab/trop/eg2-fp-trop.eps")
...
df <- read.csv("SAS/trop/TROPresult2-K2000-R1.csv")
...
ldg2a <- c("Estimate without PB",
           ...
           "95% lower confidence limit of Zhou et al.'s method")
dfc1 <- data.frame(
  p = rep(df$p[1:10],4),
...
ggsave(filename = "fig-tab/trop/eg2-trop2.eps", plot = p1, device = cairo_ps, width = 18, height = 18)
...
colnames(tab1) <- c("$p$", "$M$","$(c_1^2,c_2^2)=(1,0)$", "$(c_1^2,c_2^2)=(0,1)$", "$(c_1^2,c_2^2)=(0.5,0.5)$",
...
## 
b.MC.result1 <- read.csv("SAS/trop/TROPresult2-K1000-R1.csv")
b.MC.result2 <- read.csv("SAS/trop/TROPresult2-K2000-R1.csv")
b.MC.result3 <- read.csv("SAS/trop/TROPresult2-K5000-R1.csv")
b.MC.result4 <- read.csv("SAS/trop/TROPresult2-K20000-R1.csv")
...
p3a <- .plot.kest(
  dfk=dfk2a, lgd=lgd3, ylim = seq(0.4,0.75,0.05),
...
p3b <- .plot.kest(
  dfk=dfk2b, lgd=lgd3, ylim = seq(0.4,0.75,0.05),
...
p3c <- .plot.kest(
  dfk=dfk2c, lgd=lgd3, ylim = seq(0.4,0.75,0.05),
  title="C. Lower bounds of the SAUC under Condition (D4.3)")

## COMPARISON OF SETTING DIFFERENT K AND SAMPLES ----
##
b.MC.rep1 <- read.csv("SAS/trop/TROPresult2-K1000-R10.csv")
b.MC.rep2 <- read.csv("SAS/trop/TROPresult2-K2000-R10.csv")
b.MC.rep3 <- read.csv("SAS/trop/TROPresult2-K5000-R10.csv")
b.MC.rep4 <- read.csv("SAS/trop/TROPresult2-K20000-R10.csv")
...
p3e <- .plot.kbox(
  dfk=dfr2b, lgd=lgd3, ylim = seq(0.4,0.75,0.05),
...
p3f <- .plot.kbox(
  dfk=dfr2c, lgd=lgd3, ylim = seq(0.4,0.75,0.05),
  title="F. 10-time lower bounds of the SAUC under Condition (D4.3)")
p3 <- grid.arrange(p3a, p3b, p3c, p3d, p3e, p3f, ncol=3)
ggsave(filename = "fig-tab/trop/eg2-k-trop.eps", plot = p3, device = cairo_ps, width = 18, height = 12)
