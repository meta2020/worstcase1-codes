...
## DATA PREPROCESS
example2 <- read.csv("example-data/eg2-trop.csv")
...
# text(x=1:20, y = 0.1+sort(1/example2.2$v1), order(1/example2.2$v1))
# plot(sort(1/example2.2$v2))
# text(x=1:20, y = 0.01+sort(1/example2.2$v2), order(1/example2.2$v2))
...
df <- read.csv("SAS/TROPresult2-K2000-R1.csv")
...
lgd1 <- c("Estimate withou PB","Condition (D4.1)","Condition (D4.2)","Condition (D4.3)",
           "95% CLB (Condition D4.1)","95% CLB (Condition D4.2)","95% CLB (Condition D4.3)")
dfl <- data.frame(
  p = c(df$p[1:10],df$p,df$p),
  est = c(rep(fit$sauc.ci[1],10),df$sauc.lb, df$sauc.clb),
...
saucd <- ggplot(dfl, aes(x = p, y = est, color = grp, linetype = grp)) +
  ...
  scale_colour_manual("",
                      breaks = lgd1,
                      values = c("grey50","#4daf4a", "#e41a1c", "#377eb8","#4daf4a", "#e41a1c", "#377eb8"))+
  scale_linetype_manual("",
                        breaks = lgd1,
                        values = c(2,1,2,3,3,3,3))+
  ggtitle("D. Lower bounds of the SAUC under 3 conditions")
p1 <- grid.arrange(srocd1, srocd2, srocd3, saucd, ncol=2)
ggsave(filename = "fig-tab/trop/eg2.eps", plot = p1, device = cairo_ps, width = 12, height = 12)

## TABLE 1: LOWER BOUND OF THE SAUC ----

tab12 <- dfl %>% spread(key = grp, value = est)
tab1 <- cbind.data.frame(p =　seq(1, 0.1, -0.1),
                         A = sprintf("%.3f (%.3f)", tab12$`Condition (D4.1)`, tab12$`95% CLB (Condition D4.1)`),
                         B = sprintf("%.3f (%.3f)", tab12$`Condition (D4.2)`, tab12$`95% CLB (Condition D4.2)`),
                         C = sprintf("%.3f (%.3f)", tab12$`Condition (D4.3)`, tab12$`95% CLB (Condition D4.3)`))
colnames(tab1) <- c("$p$", "LB (95\\% CLB)", "LB (95\\% CLB)", "LB (95\\% CLB)")

sink("fig-tab/trop/tab2.tex")
kbl(tab1[order(tab1[,1], decreasing = T),],
    format = "latex",
    longtable = F,
    booktabs = T,
    linesep = "",
...
load("rfunc/comp-trop-dt.RData")

## PLOT 2a: c1=1, SENSITIVITY ----
## 
ldg2a <- c("Estimate withou PB",
           "Simulation-based bound (Condition D4.1): \n PB is related with variances of the logit-sensitivity",
           "95% CLB (simulation-based bound)",
           "Zhou et al.'s method (c1=1, c2=0): \n PB is related with p-values of the logit-sensitivity",
           "95% CLB (Zhou et al.'s method)")
dfc1 <- data.frame(
  p = rep(df$p[1:10],5),
  est = c(dfl[dfl$grp %in% c("Estimate withou PB","Condition (D4.1)","95% CLB (Condition D4.1)"),"est"],
...
p2a <- ggplot(dfc1, aes(x = p, y = est, color = grp, linetype = grp)) +
  ...
  scale_colour_manual("",
                      breaks = ldg2a,
                      values = c("grey50","#e41a1c","#e41a1c","#377eb8","#377eb8"))+
  scale_linetype_manual("",
                        breaks = ldg2a,
                        values = c(2,1,3,1,3))+
...
ldg2b <- c("Estimate withou PB",
           "Simulation-based bound (Condition D4.2): \n PB is related with variances of the logit-specificity",
           "95% CLB (simulation-based bound)",
           "Zhou et al.'s method (c1=0, c2=1): \n PB is related with p-values of the logit-specificity",
           "95% CLB (Zhou et al.'s method)")
dfc2 <- data.frame(
  p = rep(df$p[1:10],5),
  est = c(dfl[dfl$grp %in% c("Estimate withou PB","Condition (D4.2)","95% CLB (Condition D4.2)"),"est"],
...
ldg2c <- c("Estimate withou PB",
           "Simulation-based bound (Condition D4.3): \n PB is related with variances of the lnDOR",
           "95% CLB (simulation-based bound)",
           "Zhou et al.'s method (c1=c2): \n PB is related with p-values of the lnDOR",
           "95% CLB (Zhou et al.'s method)")
dfc3 <- data.frame(
  p = rep(df$p[1:10],5),
  est = c(dfl[dfl$grp %in% c("Estimate withou PB","Condition (D4.3)","95% CLB (Condition D4.3)"),"est"],
...
lgd2d <- c("Estimate withou PB",
           ...
           "95% CLB (simulation-based bound)",
           "Zhou et al.'s method (estimated c1 c2)",
           "95% CLB (Zhou et al.'s method)")
dfl1 <- dfl[dfl$grp %in% c("Estimate withou PB","Condition (D4.1)","95% CLB (Condition D4.1)"),]
dfl2 <- dfl[dfl$grp %in% c("Condition (D4.2)","95% CLB (Condition D4.2)"),]
dfl3 <- dfl[dfl$grp %in% c("Condition (D4.3)","95% CLB (Condition D4.3)"),]
...
p2d <- ggplot(dfl4, aes(x = p, y = est1, color = grp2, linetype = grp2)) +
  ...
  scale_linetype_manual("",
                        ...
                        values = c(2,1,3,1,3,0,0)) +
  scale_shape_manual("",
                     breaks = c(lgd2d, "Piao et al.'s method","Li et al.'s method"),
                     values = c(rep(0,5),16,18))

p2 <- grid.arrange(p2a, p2b, p2c, ncol=3)
ggsave(filename = "fig-tab/trop/eg2-comp1.eps", plot = p2, device = cairo_ps, width = 18, height = 6)
p2d1 <- p2d+ggtitle("Comparison with other methods")
ggsave(filename = "fig-tab/trop/eg2-comp2.eps", plot = p2d1, device = cairo_ps, width = 8, height = 8)
p2d2 <- p2d+ggtitle("D. Comparison scenario 4")
p24 <- grid.arrange(p2a, p2b, p2c, p2d2, ncol=2)
ggsave(filename = "fig-tab/trop/eg2-comp.eps", plot = p24, device = cairo_ps, width = 12, height = 12)


## COMPARISON OF SETTING DIFFERENT K ----
## 
b.MC.result1 <- read.csv("SAS/TROPresult2-K1000-R1.csv")
b.MC.result2 <- read.csv("SAS/TROPresult2-K2000-R1.csv")
b.MC.result3 <- read.csv("SAS/TROPresult2-K5000-R1.csv")
b.MC.result4 <- read.csv("SAS/TROPresult2-K20000-R1.csv")
...
p3a <- ggplot(dfk2a, aes(x = p, y = est, color = grp, linetype = grp)) +
  geom_line(size=1) +
  scale_y_continuous(limits = c(0.4,0.8), name = "SAUC") +
  scale_x_reverse(n.breaks = 10, name="Marginal selection probability") + 
  theme(panel.background = element_rect(fill = "white", colour = "grey50"),
        panel.grid.major = element_line(colour = "grey87"),
        legend.key = element_rect (fill = "white"),
...
##
b.MC.rep1 <- read.csv("SAS/TROPresult2-K1000-R10.csv")
b.MC.rep2 <- read.csv("SAS/TROPresult2-K2000-R10.csv")
b.MC.rep3 <- read.csv("SAS/TROPresult2-K5000-R10.csv")
b.MC.rep4 <- read.csv("SAS/TROPresult2-K20000-R10.csv")
...
ggsave(filename = "fig-tab/trop/eg2-k.eps", plot = p3, device = cairo_ps, width = 18, height = 12)
