...
assumption = 1);
	...
	t1 = log(se/(1-se))/sqrt(v11);
	t2 = log(sp/(1-sp))/sqrt(v22);
	t3 = log(lndor)/sqrt(v3);

	run;

	/*ASSUMPTION 2*/
	%if &assumption = 1 %then %do;
		proc sort data = dat_v; by descending t1 t2; run;
	%end;
	%if &assumption = 2 %then %do;
		proc sort data = dat_v; by descending t2 t1; run;
	%end;
	%if &assumption = 3 %then %do;
		proc sort data = dat_v; by descending t3 t1 t2; run;
	%end;
