...
%macro bias1opt(start=0.1, end=1, by=0.1, plvl=0, m=3);
%local i;
...
proc optmodel Printlevel= &plvl;
	...
	/* ASSUMPTION1 */
	con cons5 {i in 1..(n-1), j in 1..&m}: 			psk[i,j]<=psk[i+1,j]; 
	con cons6 {i in 1..(n-1), j in (&m+1)..k}: 			psk[i,j]>=psk[i+1,j]; 

	/* MAXIMUM OF b*/
/*	max maxb = (1/si2)* sum{i in 1..n}(1/precision[i]^2*(sum{j in 1..k} (1/precision[i])^2*sqrt(6.28)*exp(-0.5*probit(psk[i,j])*probit(psk[i,j])))/(k*ps[i]));*/
	max maxb = 1/si2*(1/(k))*(sum{i in 1..n, j in 1..k} precision[i]*zk[j]*psk[i,j]/ps[i]);
	solve;
	status = _STATUS_;
	solstatus = _SOLUTION_STATUS_;
	create data maxb from p=&p k=k maxb=maxb status1=status solstatus1=solstatus m=&m;
...
data dat_k; k=&k; run;
/*data dat_m; do sequence=2 to (&k-2); output; end; run;*/
%local i;
%do i=1 %to &repeatn;
	...
	/*PREPARE RESULT DATA*/
	data result; run;
	%bias1opt(start=&start, end=&end, by=&by, plvl=&plvl, m=&m);
