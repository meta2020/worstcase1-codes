...
df <- read.csv("SAS/result2-2000trop.csv")
df.as1 <- df[df$as==1,]
df.as2 <- df[df$as==2,]
df.as3 <- df[df$as==3,]
...
ggsave(filename = "fig-tab/eg2-1.eps", plot = p21, device = cairo_ps, width = 12, height = 12)
...
ggsave(filename = "fig-tab/comp-trop.eps", plot = p3, device = cairo_ps, width = 8, height = 8)
...
ggsave(filename = "fig-tab/eg2-k.eps", plot = p2k, device = cairo_ps, width = 12, height = 12)