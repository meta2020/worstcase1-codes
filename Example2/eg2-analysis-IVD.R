##
## EXAMPLE 2 IN WEB APPENDIX IVD DATA
##
...
df <- read.csv("SAS/result2-2000ivd.csv")
df.as1 <- df[df$as==1,]
df.as2 <- df[df$as==2,]
df.as3 <- df[df$as==3,]
...
ggsave(filename = "fig-tab/eg2-1-supp.eps", plot = p21, device = cairo_ps, width = 12, height = 12)
...
ggsave(filename = "fig-tab/comp-ivd.eps", plot = p3, device = cairo_ps, width = 8, height = 8)


## PLOT 3: COMPARISON OF SETTING DIFFERENT K
b.MC.result1 <- read.csv("SASresult/result2-1000IVD.csv")
b.MC.result2 <- read.csv("SASresult/result2-2000IVD.csv")
b.MC.result3 <- read.csv("SASresult/result2-20000IVD.csv")
...
ggsave(filename = "eg2-k-IVD.eps", plot = p2k, device = cairo_ps, width = 12, height = 12)
